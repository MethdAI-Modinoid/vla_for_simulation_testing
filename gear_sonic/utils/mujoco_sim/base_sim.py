...
class DefaultEnv:
    ...
    def __init__(
        ...
        if not camera_configs and offscreen and enable_image_publish:
            # Publish keys must match the data/VLA pipeline feature names
            # (observation.images.<key>); mjcf_name is the actual MuJoCo camera to
            # render (see the <camera> tags in g1_29dof_with_hand.xml).
            self.camera_configs = {
                "ego_view": {"height": 480, "width": 640, "mjcf_name": "ego_view"},
                "ego_left": {"height": 480, "width": 640, "mjcf_name": "ego_view_left_mono"},
                "ego_right": {"height": 480, "width": 640, "mjcf_name": "ego_view_right_mono"},
            }
    ...
    def init_scene(self):
        ...
        if self.config["ENABLE_ELASTIC_BAND"] and self.use_floating_root_link:
            ...
            # AUTO-RELEASE: an external SIGUSR1 releases the hanger, equivalent to pressing
            # '9' in the GUI. Used by run_sim.sh to stand the robot without a GUI keypress.
            # Harmless if the signal is never sent.
            import signal as _signal

            def _release_elastic_band(signum, frame):
                self.elastic_band.enable = False
                print("ElasticBand enable: False (via SIGUSR1)", flush=True)

            try:
                _signal.signal(_signal.SIGUSR1, _release_elastic_band)
            except (ValueError, OSError):
                pass  # not the main thread; auto-release unavailable
        ...
        self._init_manipulation_cubes()

    def _init_manipulation_cubes(self):
        """Cache freejoint qpos addresses and initial poses for the colored cubes.

        The cubes only exist in scenes that define them (e.g. scene_43dof.xml); if
        the joints are absent this is a no-op so other scenes are unaffected.
        """
        # Reset handling: Backspace (from the viewer's UI thread) sets this flag,
        # and sim_step detects any reset by the sim clock rewinding to 0.
        self._reset_requested = False
        self._prev_sim_time = 0.0

        cube_joint_names = ["cube_red_free", "cube_green_free", "cube_blue_free"]
        existing = [
            n
            for n in cube_joint_names
            if mujoco.mj_name2id(self.mj_model, mujoco.mjtObj.mjOBJ_JOINT, n) != -1
        ]
        if len(existing) != len(cube_joint_names):
            self.cube_qpos_adr = None
            return

        # qpos address of each cube's freejoint (7 values: x,y,z, qw,qx,qy,qz).
        self.cube_qpos_adr = [self.mj_model.joint(n).qposadr[0] for n in cube_joint_names]

        # Each cube's initial pose, taken from the model (i.e. the body positions
        # tuned in the scene XML for the robot's arm reach).
        self.cube_init_qpos = [
            self.mj_model.qpos0[adr : adr + 7].copy() for adr in self.cube_qpos_adr
        ]
        self.cube_xy_jitter = 0.05  # +/- 5 cm in x and y

        # The 3 tuned start positions, ordered by y as slots A, B, C (nearest the
        # red, green, blue baskets respectively).
        slot_order = np.argsort([q[1] for q in self.cube_init_qpos])
        self.cube_slot_qpos = [self.cube_init_qpos[i] for i in slot_order]

        # Balanced schedule: cycle through all 3! = 6 cube->slot arrangements,
        # EPISODES_PER_CONFIG episodes each (6 x 16 = 96), so every permutation is
        # collected equally instead of relying on random sampling. Each tuple gives
        # the cube index (red=0, green=1, blue=2) placed at slots (A, B, C).
        self.cube_configs = [
            (0, 1, 2),  # 1: A=red   B=green  C=blue
            (0, 2, 1),  # 2: A=red   B=blue   C=green
            (1, 0, 2),  # 3: A=green B=red    C=blue
            (1, 2, 0),  # 4: A=green B=blue   C=red
            (2, 0, 1),  # 5: A=blue  B=red    C=green
            (2, 1, 0),  # 6: A=blue  B=green  C=red
        ]
        self.episodes_per_config = 16
        self._episode_count = 0

    def randomize_cubes(self):
        """Place cubes per the balanced 6-config schedule, plus +/-5 cm jitter.

        Cycles deterministically through all 6 cube arrangements (16 episodes each,
        96 total) so every permutation is collected equally; the +/-5 cm x/y jitter
        still varies within each episode. After 96 episodes the schedule repeats.
        """
        if getattr(self, "cube_qpos_adr", None) is None:
            return

        config_idx = (self._episode_count // self.episodes_per_config) % len(self.cube_configs)
        config = self.cube_configs[config_idx]
        for slot_idx, cube_idx in enumerate(config):
            adr = self.cube_qpos_adr[cube_idx]
            qpos = self.cube_slot_qpos[slot_idx].copy()
            #qpos[0] += np.random.uniform(-self.cube_xy_jitter, self.cube_xy_jitter)
            
            #qpos[1] += np.random.uniform(-self.cube_xy_jitter, self.cube_xy_jitter)
            self.mj_data.qpos[adr : adr + 7] = qpos

        print(
            f"[cube reset] episode {self._episode_count}, config {config_idx + 1}/6, "
            f"cubes at (A,B,C) = {config}"
        )
        self._episode_count += 1

    def _viewer_key_callback(self, keycode):
        """Viewer key handler (runs on the viewer's UI thread).

        Keep it minimal and thread-safe: only flip flags / scalar state here, and
        let sim_step (physics thread) do the actual mjData work. Preserves the
        elastic-band keys (7/8/9, including 9 = release from hanger).
        """
        import glfw

        band = getattr(self, "elastic_band", None)
        if band is not None:
            band.MujuocoKeyCallback(keycode)
    ...
    def sim_step(self):
        # Backspace (captured on the viewer's UI thread) requests a reset; perform
        # it here on the physics thread.
        if self._reset_requested:
            self._reset_requested = False
            self.reset()
        # Any reset (Backspace, fall, programmatic, or the viewer's built-in) rewinds
        # the sim clock to 0. When we detect that, reshuffle the cubes so the scene
        # changes on every reset.
        if self.mj_data.time < self._prev_sim_time - 1e-9:
            self.randomize_cubes()
            mujoco.mj_forward(self.mj_model, self.mj_data)
        self._prev_sim_time = self.mj_data.time
    ...
    def reset(self):
        # Cube randomization is handled centrally by sim_step's reset detection
        # (the sim clock rewinds to 0), which also covers the viewer's built-in reset.
