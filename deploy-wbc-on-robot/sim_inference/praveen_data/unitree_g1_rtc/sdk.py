...
class ROSObservationReader:
    ...
    def get_observation(self, lang_instruction: str = "") -> Optional[Dict[str, Any]]:
        ...
        # Main ego view camera (RGB)
        if "ego_view" in images:
            obs["ego_view"] = images["ego_view"]
        
        if self.add_stereo_camera:
            # Left stereo camera
            if "ego_view_left_mono" in images:
                obs["ego_view_left_mono"] = images["ego_view_left_mono"]
            
            # Right stereo camera
            if "ego_view_right_mono" in images:
                obs["ego_view_right_mono"] = images["ego_view_right_mono"]
