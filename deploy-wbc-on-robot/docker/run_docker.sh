...
function get_project_dir {
    # When --root is used, always use the fixed project directory
    # instead of relying on git-based auto-detection.
    if [ "$ROOT" = true ]; then
...
if [ "$DEPLOY" = true ]; then
    ...
    PROJECT_MOUNT_DEST="$DOCKER_HOME_DIR/Projects/$WORKTREE_NAME"

    # If a bash container already exists, make sure it is bound to the CURRENT
    # project directory before reusing it. Mounts and working directory are fixed
    # at container-creation time, so `docker start`/`exec` on a container that was
    # created for a different repo/path silently drops you into the wrong project.
    if sudo docker ps -a --format '{{.Names}}' | grep -q "^$BASH_CONTAINER$"; then
        EXISTING_SRC=$(sudo docker inspect "$BASH_CONTAINER" \
            --format "{{range .Mounts}}{{if eq .Destination \"$PROJECT_MOUNT_DEST\"}}{{.Source}}{{end}}{{end}}" 2>/dev/null)
        if [ "$EXISTING_SRC" = "$PROJECT_DIR" ]; then
            echo "Bash container exists, starting it..."
            sudo docker start $BASH_CONTAINER > /dev/null
            sudo docker exec -it -w "$PROJECT_MOUNT_DEST" $BASH_CONTAINER /bin/bash
        else
            echo "Existing bash container is bound to a different project path; recreating..."
            echo "  expected: $PROJECT_DIR -> $PROJECT_MOUNT_DEST"
            echo "  found:    ${EXISTING_SRC:-<no matching mount>}"
            sudo docker rm -f $BASH_CONTAINER > /dev/null
        fi
    fi

    # Create the container if it does not exist (or was just removed above).
    if ! sudo docker ps -a --format '{{.Names}}' | grep -q "^$BASH_CONTAINER$"; then
