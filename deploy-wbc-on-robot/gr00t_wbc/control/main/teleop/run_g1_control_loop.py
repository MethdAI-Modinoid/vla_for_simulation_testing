...
from gr00t_wbc.control.utils.hand_grasp_smoother import HandGraspSmoother
...
def main(config: ControlLoopConfig):
    ...
    hand_smoother = None
    if config.smooth_hand_grasp:
        hand_smoother = HandGraspSmoother(dt=1.0 / config.control_frequency)
        print(
            "Hand grasp smoothing ENABLED: right-hand joints (36..42) slew-limited "
            "to the finger envelope (~6 rad/s) before execution and recording."
        )
    ...
    try:
        while ros_manager.ok():
            ...
            with telemetry.timer("total_loop"):
                ...
                # Smooth the right-hand grasp before it is both executed
                # (env.queue_action below) and recorded (action publish), so the
                # collected action label matches what the robot actually does.
                if hand_smoother is not None:
                    hand_smoother.smooth(wbc_action["q"])
                ...
                if env.use_sim and wbc_goal.get("reset_env_and_policy", False):
                    ...
                    # Re-seed the grasp smoother from the measured hand pose so
                    # the post-reset command has no artificial jump.
                    if hand_smoother is not None:
                        hand_smoother.reset(obs["q"])
