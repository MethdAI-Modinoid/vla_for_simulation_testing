...
class ControlLoopConfig(BaseConfig):
    ...
    smooth_hand_grasp: bool = True
    """Rate-limit the right-hand grasp (Dex3, joints 36..42) to ~6 rad/s in the
    control loop so the robot executes the smoothed command and the recorded
    `action` label matches it (finger envelope ~4.7-9 rad/s). Disable with
    --no-smooth_hand_grasp."""
...
class DataExporterConfig(BaseConfig, ComposedCameraClientConfig):
    ...
    """Host IP of the ZMQ VLA camera server (overrides the localhost default)."""
    ...
    """Whether to add the extra stereo-mono ego views. Kept False for the 3-camera
    VLA setup (ego_view + ego_left + ego_right); the ZMQ client does not publish the
    stereo-mono pair, so enabling this would require image keys that never arrive."""
