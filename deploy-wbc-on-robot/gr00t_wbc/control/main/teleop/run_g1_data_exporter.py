...
class Gr00tDataCollector:
    def __init__(
        ...
        self._last_wait_log = 0.0  # throttle the "waiting for message" diagnostic
    ...
    def _add_data_frame(self):
        ...
        if self.latest_proprio_msg is None or self.latest_image_msg is None:
            # Throttle to ~1 Hz so the diagnostic doesn't flood the console.
            now = time.monotonic()
            if now - self._last_wait_log >= 1.0:
                self._last_wait_log = now
                img_detail = ""
                # debug_status() may be absent if an older installed VLAImageClient
                # shadows the repo copy -- degrade gracefully instead of crashing.
                debug_status = getattr(self._image_subscriber, "debug_status", None)
                if self.latest_image_msg is None and debug_status is not None:
                    status = debug_status()
                    if status["total_msgs"] == 0:
                        img_detail = (
                            " | NO ZMQ frames received -> camera server is not "
                            "publishing or is unreachable at the configured "
                            "camera_host:camera_port"
                        )
                    else:
                        img_detail = (
                            f" | received {status['total_msgs']} ZMQ msgs but still "
                            f"waiting on cameras {status['missing_keys']}; "
                            f"topics seen={status['seen_topics']} "
                            f"expected={status['expected_topics']}"
                        )
                self._print_and_say(
                    f"Waiting for message. "
                    f"Avail msg: proprio {self.latest_proprio_msg is not None} | "
                    f"image {self.latest_image_msg is not None}{img_detail}",
                    say=False,
                )
            return False
...
if __name__ == "__main__":
    ...
    while add_to_existing_dataset not in ("y", "n"):
        add_to_existing_dataset = (
            input("Please enter 'y' or 'n'. Add to existing dataset? (y/n): ").strip().lower()
        )
