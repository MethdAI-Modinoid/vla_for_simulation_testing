...
class JointSafetyMonitor:
    ...
    ARM_VELOCITY_LIMIT = 15.0  # rad/s for arm joints
    HAND_VELOCITY_LIMIT = 200.0  # rad/s for finger joints
    ...
    def _initialize_limits(self):
        ...
        # Hand joints: velocity monitoring disabled - readings are too noisy for reliable limits
        # Position control handles hand safety via hardware limits
    ...
    def check_safety(self, obs: Dict, action: Dict) -> Tuple[bool, List[Dict]]:
        ...
        # Skip velocity checks during startup ramp - sensor readings can be noisy
        # during the first steps as the robot transitions from its initial state.
    ...
    def get_violation_report(self, violations: Optional[List[Dict]] = None) -> str:
        ...
        if critical_violations:
            report += "Action: Safety shutdown disabled - violation logged only. Robot continues operation."
        else:
    ...
    def handle_violations(self, obs: Dict, action: Dict) -> Dict:
        ...
        # Critical velocity violations: shutdown has been intentionally disabled.
        # We still detect, report, and log them for telemetry/diagnostics, but we no
        # longer engage safe mode or exit the process - the robot keeps operating.
...
def main():
    ...
    try:
        ...
        safe_obs = {
            ...
            "dq": np.zeros(robot_model.num_dofs),
        }
