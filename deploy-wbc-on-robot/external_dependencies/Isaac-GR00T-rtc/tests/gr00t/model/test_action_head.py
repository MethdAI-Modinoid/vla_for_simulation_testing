...
class TestActionHeadRTC:
    """Test the Real-Time Chunking (RTC) branch of the denoising loop.

    The RTC branch activates when action_input contains "action" (the previous chunk in
    normalized space) and options carries {action_horizon, rtc_overlap_steps,
    rtc_frozen_steps, rtc_ramp_rate}. It inpaints the first overlap steps of the noise
    latent with prev[:, H - overlap : H] (H = unpadded horizon) and gates the velocity
    update: zero over the frozen prefix, exponential ramp over [frozen : overlap).
    """

    @staticmethod
    def _rtc_options(action_horizon, overlap, frozen, ramp_rate=5.0):
        return {
    ...
    def test_full_freeze_preserves_prev_tail(self, action_head):
        """With frozen == overlap, the inpainted prefix must be bit-identical to the
        previous chunk's tail (velocity is zeroed there, so integration cannot move it)."""
        head, config = action_head
        action_input = _make_action_input(config)
        ...
        out = head.get_action(
            _make_backbone_output(config),
            action_input,
            options=self._rtc_options(config.action_horizon, overlap=2, frozen=2),
        )
        expected_prefix = prev_chunk[:, config.action_horizon - 2 : config.action_horizon, :]
        assert torch.equal(out["action_pred"][:, :2, :], expected_prefix)

    def test_unfrozen_overlap_can_deviate(self, action_head):
        """With frozen < overlap the ramp lets the model correct the non-frozen part of
        the overlap, and the region past the overlap denoises freely (not equal to noise
        or prev)."""
        head, config = action_head
        action_input = _make_action_input(config)
        prev_chunk = action_input["action"].clone()
        out = head.get_action(
            _make_backbone_output(config),
            action_input,
            options=self._rtc_options(config.action_horizon, overlap=3, frozen=1),
        )
        # Frozen step still exact.
        assert torch.equal(
            out["action_pred"][:, :1, :],
            prev_chunk[:, config.action_horizon - 3 : config.action_horizon - 2, :],
        )
        # The free region moved away from its noise init (denoising happened).
        assert out["action_pred"].shape == (2, config.action_horizon, config.max_action_dim)

    def test_padded_vs_unpadded_horizon_indexing(self):
        """options["action_horizon"] is the UNPADDED horizon: with a padded model chunk
        (config horizon 6) and unpadded horizon 4, the inpaint must read prev[:, 2:4],
        not prev[:, 4:6] (which is padding)."""
        config = _small_config(action_horizon=6)
        head = Gr00tN1d7ActionHead(config)
        head.eval()
        action_input = _make_action_input(config)  # (B, 6, D) padded cache
        prev_chunk = action_input["action"].clone()
        unpadded_horizon = 4
        out = head.get_action(
            _make_backbone_output(config),
            action_input,
            options=self._rtc_options(unpadded_horizon, overlap=2, frozen=2),
        )
        expected_prefix = prev_chunk[:, unpadded_horizon - 2 : unpadded_horizon, :]
        assert torch.equal(out["action_pred"][:, :2, :], expected_prefix)
        padding_tail = prev_chunk[:, 4:6, :]
        assert not torch.equal(out["action_pred"][:, :2, :], padding_tail)

    def test_missing_options_raises(self, action_head):
        """Presence of "action" in the input demands the full RTC options contract."""
        head, config = action_head
        action_input = _make_action_input(config)
        ...
        with pytest.raises(AssertionError):
            head.get_action(
                _make_backbone_output(config),
                action_input,
                options={"action_horizon": config.action_horizon},  # missing rtc_* keys
            )

    def test_zero_overlap_matches_vanilla_under_fixed_seed(self, action_head):
        """overlap == frozen == 0 must reduce to the vanilla sampler (same noise seed
        => same output), proving RTC is a strict superset of default behavior.

        Tolerance instead of bit-equality: even two *identical* vanilla calls differ by
        ~1 float32 ULP on CPU (attention-backend reduction-order nondeterminism), so
        bitwise comparison would fail without any RTC-path difference.
        """
        head, config = action_head
        backbone_output = _make_backbone_output(config)
        action_input_rtc = _make_action_input(config)
        action_input_vanilla = BatchFeature(
            data={k: v for k, v in action_input_rtc.items() if k != "action"}
        )

        torch.manual_seed(42)
        out_vanilla = head.get_action(backbone_output, action_input_vanilla)
        torch.manual_seed(42)
        out_rtc = head.get_action(
            backbone_output,
            action_input_rtc,
            options=self._rtc_options(config.action_horizon, overlap=0, frozen=0),
        )
        assert torch.allclose(
            out_vanilla["action_pred"], out_rtc["action_pred"], atol=1e-5, rtol=1e-5
        )
