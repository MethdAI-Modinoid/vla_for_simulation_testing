...
class MockPolicy:
    ...
    def get_action(self, observation, options=None):
        ...
        # Echo the received options so transport tests can assert they arrive intact.
        action = {"joint_pos": np.zeros(7, dtype=np.float32)}
        info = {"mock": True, "received_options": options}
        return action, info
...
class TestPolicyServerClient:
    ...
    def test_nested_rtc_options_roundtrip(self, server_client):
        """RTC clients send options["rtc"] = {...} with each request; the nested dict
        (plain ints/floats/bools) must reach the server-side policy unchanged."""
        client, _, _ = server_client
        obs = {"state": {"joint_pos": np.zeros(7, dtype=np.float32)}}
        rtc_options = {
            "rtc": {
        ...
        _, info = client.get_action(obs, rtc_options)
        assert info["received_options"] == rtc_options
