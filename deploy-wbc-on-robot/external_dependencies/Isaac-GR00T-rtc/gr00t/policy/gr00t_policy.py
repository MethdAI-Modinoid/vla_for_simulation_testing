...
from .rtc import (  # noqa: F401  (RTCConfig re-exported for consumers)
    RTCConfig,
...
class Gr00tPolicy(BasePolicy):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            rtc_config: Optional Real-Time Chunking configuration. When set, the policy
                caches each predicted chunk and seeds the next inference with its
                unexecuted remainder (inpainting), removing chunk-boundary
                discontinuities. Assumes a single control loop per policy instance;
                call reset() between episodes. See gr00t/policy/rtc.py.
        """
        ...
        # Real-Time Chunking state (see gr00t/policy/rtc.py). The cache holds the previous
        # normalized, padded action chunk exactly as the model produced it, so it can be fed
        # back for inpainting without any re-normalization.
        self.rtc_config = rtc_config
        self._rtc_prev_action: torch.Tensor | None = None
        if self.rtc_config is not None:
            self._validate_rtc_compatibility()

    def _validate_rtc_compatibility(self) -> None:
        """Refuse RTC when the checkpoint decodes RELATIVE action groups against the
        current state: the cached chunk is anchored to the previous inference's state, so
        inpainting it would shift the reference frame by the state motion between queries.
        """
        assert self.rtc_config is not None
        use_relative_action = bool(getattr(self.processor, "use_relative_action", False))
        action_configs = self.modality_configs["action"].action_configs or []
        relative_keys = [
            key
            for key, cfg in zip(self.modality_configs["action"].modality_keys, action_configs)
            if cfg.rep == ActionRepresentation.RELATIVE
        ]
        if use_relative_action and relative_keys and not self.rtc_config.allow_relative:
            raise ValueError(
                f"RTC is disabled for this checkpoint: use_relative_action=True and action "
                f"groups {relative_keys} use RELATIVE representation, which is decoded "
                f"against the current state. Inpainting the previous chunk would apply a "
                f"stale reference frame. Set RTCConfig(allow_relative=True) to override if "
                f"the inter-query state motion is negligible."
            )
    ...
    def _get_action(
        ...
        Args:
            ...
            options: Optional parameters. When RTC is configured, supports
                options["rtc"] = {"executed_steps": int, "enabled": bool} to control
                chunk blending per call (see gr00t/policy/rtc.py).
        ...
        # With RTC configured, seed the new chunk with the previous chunk's unexecuted
        # remainder (inpainting) so consecutive chunks join without discontinuities.
        rtc_model_options = None
        if self.rtc_config is not None:
            rtc_model_options = self._prepare_rtc_injection(
                collated_inputs, options, batch_size=len(unbatched_observations)
            )

        with torch.inference_mode():
            if self.rtc_config is not None:
                model_pred = self.model.get_action(**collated_inputs, options=rtc_model_options)
            else:
        ...
        info: dict[str, Any] = {}
        if self.rtc_config is not None:
            # Cache the normalized, padded chunk exactly as produced — the format the RTC
            # inpaint slot expects on the next call.
            self._rtc_prev_action = model_pred["action_pred"].detach().clone()
            info["rtc_applied"] = rtc_model_options is not None
            if rtc_model_options is not None:
                info["rtc_overlap_steps"] = rtc_model_options["rtc_overlap_steps"]
                info["rtc_frozen_steps"] = rtc_model_options["rtc_frozen_steps"]
    ...
    def _prepare_rtc_injection(
        self,
        collated_inputs: dict[str, Any],
        call_options: dict[str, Any] | None,
        batch_size: int,
    ) -> dict[str, Any] | None:
        """Inject the previous chunk into the model inputs and resolve RTC options.

        Returns the resolved model-level options dict when RTC applies to this call,
        otherwise None (first call of an episode, RTC disabled per call, no cadence
        information, or invalidated cache). Mutates collated_inputs["inputs"] in place
        when RTC applies.
        """
        assert self.rtc_config is not None
        params = merge_rtc_call_options(self.rtc_config, call_options)
        if not params.enabled:
            return None
        if params.executed_steps is None:
            logger.warning(
                "RTC is configured but neither RTCConfig.execution_horizon nor "
                "options['rtc']['executed_steps'] was provided; running without RTC. "
                "Set the execution horizon to your control loop's query cadence."
            )
            return None

        prev_action = self._rtc_prev_action
        if prev_action is None:
            return None
        if prev_action.shape[0] != batch_size:
            logger.warning(
                "RTC cache invalidated: cached batch size %d != current batch size %d.",
                prev_action.shape[0],
                batch_size,
            )
            self._rtc_prev_action = None
            return None

        unpadded_horizon = len(self.modality_configs["action"].delta_indices)
        resolved = resolve_rtc_options(
            unpadded_horizon=unpadded_horizon,
            executed_steps=params.executed_steps,
            overlap_steps=params.overlap_steps,
            frozen_steps=params.frozen_steps,
            ramp_rate=params.ramp_rate,
        )
        if resolved is None:
            return None

        shift = compute_prev_chunk_shift(
            unpadded_horizon, params.executed_steps, resolved["rtc_overlap_steps"]
        )
        collated_inputs["inputs"]["action"] = shift_prev_chunk(prev_action, shift)
        return resolved
    ...
    def reset(self, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        Clears the RTC previous-chunk cache so a new episode starts from vanilla
        sampling. Call this between episodes when RTC is configured.
