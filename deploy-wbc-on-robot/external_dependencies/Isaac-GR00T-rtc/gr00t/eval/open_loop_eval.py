...
def evaluate_single_trajectory(
    ...
    # Stateful policies (RTC previous-chunk cache) must start each trajectory fresh.
    policy.reset()
    get_action_options = {"rtc": {"executed_steps": execution_horizon}} if rtc else None
    ...
    for step_count in range(0, actual_steps, execution_horizon):
        ...
        _action_chunk, _ = policy.get_action(parsed_obs, get_action_options)
        action_chunk = parse_action_gr00t(_action_chunk)
        # Keep the FULL predicted chunk (all keys concatenated per step) for the
        # chunk-boundary smoothness metrics.
        chunk_len = np.atleast_1d(action_chunk[f"action.{action_keys[0]}"]).shape[0]
    ...
    # Chunk-boundary smoothness metrics (jerk diagnostics; see gr00t/eval/chunk_metrics.py).
    # These quantify the RTC on/off difference: lower boundary_jump / intra_accel and a
    # momentum_shift cosine closer to 1 mean smoother chunk transitions.
    chunk_smoothness: dict[str, float] = {}
    if len(predicted_chunks) >= 2:
        chunk_smoothness = compute_chunk_metrics(
            np.stack(predicted_chunks), execute_steps=execution_horizon
        )
        logging.info(f"Chunk smoothness metrics (rtc={rtc}): {chunk_smoothness}")
    ...
    return mse, mae, chunk_smoothness
...
class ArgsConfig:
    ...
    rtc: bool = False
    """Enable Real-Time Chunking: seed each new chunk with the previous chunk's unexecuted
    remainder to remove chunk-boundary discontinuities. With a remote policy server, also
    start the server with --rtc."""

    rtc_overlap_steps: int | None = None
    """RTC overlap steps. Default: action_horizon - execution_horizon (maximum overlap)."""

    rtc_frozen_steps: int = 0
    """RTC frozen prefix steps (0 for this synchronous offline eval)."""
...
def main(args: ArgsConfig):
    ...
    if local_model_path is not None:
        ...
        if args.rtc:
            logging.warning(
                "--rtc with a remote policy server requires the server to be started "
                "with --rtc (RTC state lives server-side); this client will still send "
                "per-call executed_steps and per-trajectory resets."
            )
    ...
    all_mse = []
    all_mae = []
    all_chunk_metrics: list[dict[str, float]] = []
    ...
    for traj_id in args.traj_ids:
        ...
        mse, mae, chunk_smoothness = evaluate_single_trajectory(
            policy,
            dataset,
            traj_id,
            args.embodiment_tag,
        ...
        all_mae.append(mae)
        if chunk_smoothness:
            all_chunk_metrics.append(chunk_smoothness)
    ...
    if all_chunk_metrics:
        for metric_name in all_chunk_metrics[0]:
            values = np.array([m[metric_name] for m in all_chunk_metrics])
            logging.info(
                f"Average chunk {metric_name} across all trajs (rtc={args.rtc}): "
                f"{np.nanmean(values)}"
            )
    logging.info("Done")
