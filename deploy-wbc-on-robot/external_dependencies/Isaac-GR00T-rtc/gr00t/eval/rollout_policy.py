...
def _collect_rollout_episodes(
    ...
    n_envs: int,
    seed: int | None,
    get_action_options: dict | None = None,
):
    ...
    Args:
        get_action_options: Optional options dict forwarded to every
            ``policy.get_action`` call (e.g. RTC executed-steps cadence). When set,
            the policy is treated as stateful: ``policy.reset()`` is called whenever
            an episode ends so the next episode starts fresh (requires n_envs == 1;
            vectorized envs auto-reset per-env without a policy reset hook).
    """
    if get_action_options is not None and n_envs != 1:
        raise ValueError(
            f"get_action_options implies a stateful policy, which is unsafe with "
            f"vectorized autoreset; use --n-envs 1 (got n_envs={n_envs})."
        )
    ...
    try:
        while completed_episodes < n_episodes:
            actions, _ = policy.get_action(observations, get_action_options)
            next_obs, rewards, terminations, truncations, env_infos = env.step(actions)
            # Stateful policies (RTC) need a reset when an episode ends; the n_envs == 1
            # guard above makes this detection reliable. Stateless policies keep the
            # historical behavior (no reset call).
            if get_action_options is not None and any(
                bool(terminations[i]) or bool(truncations[i]) for i in range(n_envs)
            ):
                policy.reset()
    ...
    try:
        episode_successes, episode_lengths, episode_rewards, episode_infos = (
            _collect_rollout_episodes(
                env, policy, n_episodes, n_envs, seed, get_action_options=get_action_options
            )
        )
    finally:
...
def create_gr00t_sim_policy(
    ...
    if policy_client_host and policy_client_port:
        ...
        # RTC state lives server-side; enable it with the server's --rtc flags. The
        # client still forwards per-call options (executed_steps) untouched.
...
def run_gr00t_sim_policy(
    ...
    # RTC makes the policy stateful (previous-chunk cache) — vectorized envs auto-reset
    # per-env without a policy.reset() hook, so RTC requires a single env.
    rtc_config = None
    get_action_options = None
    if rtc:
        if n_envs != 1:
            raise ValueError(f"RTC requires --n-envs 1 (stateful chunk cache); got {n_envs}")
        rtc_config = RTCConfig(
            execution_horizon=n_action_steps,
            overlap_steps=rtc_overlap_steps,
            frozen_steps=rtc_frozen_steps,
            ramp_rate=rtc_ramp_rate,
        )
        get_action_options = {"rtc": {"executed_steps": n_action_steps}}
        print(f"RTC enabled: {rtc_config}")
    ...
    with engine_cleanup:
        ...
        results = run_rollout_gymnasium_policy(
            ...
            seed=seed,
            robocasa_split=robocasa_split,
            get_action_options=get_action_options,
        )
...
class RolloutConfig:
    ...
    rtc: bool = False
    """Enable Real-Time Chunking: blend each new action chunk with the previous chunk's
    unexecuted remainder to remove chunk-boundary discontinuities. Requires --n-envs 1."""

    rtc_overlap_steps: int | None = None
    """RTC overlap steps. Default: action_horizon - n_action_steps (maximum aligned overlap)."""

    rtc_frozen_steps: int = 0
    """RTC frozen prefix steps (0 for synchronous rollouts)."""

    rtc_ramp_rate: float = 5.0
    """RTC exponential ramp rate over the overlap region."""
