...
class ServerConfig:
    ...
    # Real-Time Chunking (RTC) configs — smooth chunk-boundary blending (see gr00t/policy/rtc.py)
    rtc: bool = False
    """Enable Real-Time Chunking: seed each new chunk with the previous chunk's unexecuted
    remainder to remove chunk-boundary discontinuities. Assumes a single control loop per
    server; clients should call reset() between episodes."""

    rtc_execution_horizon: int | None = None
    """Default number of chunk steps the controller executes between policy queries. Must match
    the client's query cadence; clients can override per call via
    options={"rtc": {"executed_steps": N}}."""

    rtc_overlap_steps: int | None = None
    """Steps of the new chunk seeded from the previous chunk. Default: action_horizon minus
    executed steps (the maximum aligned overlap)."""

    rtc_frozen_steps: int = 0
    """Leading steps kept exactly equal to the previous plan (covers inference latency in async
    loops). Use 0 for synchronous loops."""

    rtc_ramp_rate: float = 5.0
    """Exponential ramp rate for blending model corrections over the overlap region."""
    ...
    if config.model_path is not None:
        ...
        if config.rtc:
            rtc_config = RTCConfig(
                ...
                overlap_steps=config.rtc_overlap_steps,
