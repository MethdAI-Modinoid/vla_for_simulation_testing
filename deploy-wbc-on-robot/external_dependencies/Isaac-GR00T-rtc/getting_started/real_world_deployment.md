...
Trajectory jitter can also be quantified using these three metrics (implemented as
importable, tested functions in `gr00t/eval/chunk_metrics.py`; `open_loop_eval.py` logs
them automatically):
...
> **RTC status:** RTC is wired end-to-end in the PyTorch path. Enable it on the policy server (`run_gr00t_server.py --rtc --rtc-execution-horizon <steps executed between queries>`) or locally via `Gr00tPolicy(..., rtc_config=RTCConfig(...))` (`gr00t/policy/rtc.py`). The policy caches each normalized predicted chunk server-side and seeds the next inference with its unexecuted remainder (inpainting); clients can override the cadence per call with `options={"rtc": {"executed_steps": N}}` and must call `reset()` between episodes. For async control loops, `gr00t/policy/async_chunk_executor.py` implements the trigger/swap logic below with latency-derived frozen steps. Quantify the effect with `gr00t/eval/chunk_metrics.py` (wired into `open_loop_eval.py --rtc`) or a sim A/B via `rollout_policy.py --rtc --n-envs 1`. Caveats: one control loop per server (the chunk cache is single-stream), and RTC refuses checkpoints that decode RELATIVE action groups against the current state (`use_relative_action=True`) unless `RTCConfig(allow_relative=True)`. The TensorRT denoising loop (`scripts/deployment/trt_model_forward.py`) does not implement RTC yet.
...
Below is a simplified async inference + RTC loop:
...
A ready-made implementation of this loop is provided by `AsyncChunkExecutor`
(`gr00t/policy/async_chunk_executor.py`). It additionally handles the details the
pseudocode glosses over: the swap enters the new chunk at the temporal offset matching the
steps executed while inference ran (so no step is replayed or skipped), and the frozen
prefix is derived from an EMA of the measured round-trip latency. Robot-side usage:

```python
from gr00t.policy.async_chunk_executor import AsyncChunkExecutor
from gr00t.policy.server_client import PolicyClient

# Server: python gr00t/eval/run_gr00t_server.py --model-path <ckpt> \
#     --embodiment-tag <tag> --rtc
policy = PolicyClient(host=SERVER_IP, port=5555)
